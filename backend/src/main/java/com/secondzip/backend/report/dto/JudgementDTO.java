...
/**
 * 개별 판정 결과. 위험도와 데이터 상태를 한 쌍으로 묶음.
 *
 * 값 자체가 정체성인 불변 객체(Value Object), 식별자가 없고,
 * 두 필드가 같으면 같은 판정으로 취급.
 *
 * 이 둘을 따로 반환하면 "CAUTION인데 데이터 상태를 안 실어 보내는" 실수가 나기 쉬워
 * 항상 함께 다니도록 묶음.
 */
public record JudgementDTO(RiskLevel riskLevel, DataStatus dataStatus) {
     ...
     * 이 매물에 해당하지 않는 항목.
     *
     * 집계에서는 제외되므로 여기 담긴 위험도는 표시용일 뿐이다.
     * SAFE로 두면 dataStatus를 읽지 않는 클라이언트에서
     * "확인하지도 않은 항목"이 "안전"으로 표시되어 잘못된 안심을 준다.
     * 그래서 기존 화면과 동일하게 CAUTION으로 둔다.
     *
     * 클라이언트가 NOT_APPLICABLE을 "해당 없음"으로 따로 표시하게 되면
     * 이 값은 무의미해진다.
     */
