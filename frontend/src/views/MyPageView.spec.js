...
const mocks = vi.hoisted(() => ({
  ...
  replace: vi.fn(),
  authStore: {
    fetchMyPage: vi.fn(), logout: vi.fn(), myPageLoading: false, loading: false,
    isAuthenticated: true,
    characterType: 'CAT',
    myPage: { email: 'user@example.com', nickname: '테스터', characterType: 'CAT' },
  },
}));
...
describe('MyPageView', () => {
  beforeEach(() => {
    ...
    mocks.getReports.mockResolvedValue({
      totalCount: 4,
      reports: [{ result: 'SAFE' }, { result: 'SAFE' }, { result: 'CAUTION' }, { result: 'DANGER' }],
    });
  });

  it('회원정보와 실제 리포트 활동 요약을 표시한다', async () => {
    const wrapper = mount(MyPageView, { global: { stubs: { RouterLink: RouterLinkStub } } });
    await flushPromises();
    expect(wrapper.text()).not.toContain('테스터 님');
    expect(wrapper.text()).toContain('ID');
    expect(wrapper.text()).toContain('user@example.com');
    expect(wrapper.findAll('.activity-card__item').map((item) => item.text())).toEqual([
      '4총 리포트', '2안전', '1주의', '1위험',
    ]);
  });
  ...
  it('각 관리 버튼을 이름 기반 하위 라우트에 연결한다', async () => {
    ...
    expect(links.map((link) => link.props('to'))).toEqual(expect.arrayContaining([
      { name: 'mypage-profile' }, { name: 'mypage-secretary' }, { name: 'mypage-withdraw' },
    ]));
  });
